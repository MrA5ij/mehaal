-- MEHAAL TECH AI - Database Schema
-- Run this in cPanel phpMyAdmin or MySQL command line

-- Create database (if not exists)
CREATE DATABASE IF NOT EXISTS mehaal_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE mehaal_db;
...
-- Projects table (Accountant, Accountant Pro, Assistant Shop)
...
-- Features table (linked to projects)
...
-- Team members table (replaces in-memory users)
...
-- Site settings (vision text, CTAs, contact emails)
...
((SELECT id FROM projects WHERE slug='accountant'), 'Voice Transaction Logging', 1),
((SELECT id FROM projects WHERE slug='accountant'), 'Instant Spoken Reports', 2),
((SELECT id FROM projects WHERE slug='accountant'), 'Zero Typing Workflow', 3),
((SELECT id FROM projects WHERE slug='accountant'), 'Multilingual Natural Understanding', 4),
((SELECT id FROM projects WHERE slug='accountant'), 'Personal Secure Vault', 5),
((SELECT id FROM projects WHERE slug='accountant'), 'Conversational Retrieval Engine', 6);

-- Insert features for Accountant Pro
INSERT INTO features (project_id, feature_name, display_order) VALUES
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Full Ledger System via Voice', 1),
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Sales, Expense & Vendor Tracking', 2),
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Profit & Cashflow Narration', 3),
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Professional PDF/CSV Reports', 4),
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Multi-Shop Access', 5),
((SELECT id FROM projects WHERE slug='accountant-pro'), 'Built-in Audit Trace', 6);

-- Insert features for Assistant Shop
INSERT INTO features (project_id, feature_name, display_order) VALUES
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Auto Inventory & Expiry Tracking', 1),
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Voice-Logged Sales & Ledgers', 2),
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Daily/Weekly Profit Snapshots', 3),
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Supplier Receiving Records', 4),
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Alerts for Stock & Expiry', 5),
((SELECT id FROM projects WHERE slug='assistant-shop'), 'Retail Intelligence Engine', 6);
